...
uint32_t FLAGS_cache_size = 40960;       // Number of bytes to use as a cache of uncompressed data.
uint32_t FLAGS_threads = 16;            // Number of concurrent threads to run.
uint32_t FLAGS_num_shard_bits = 6;      // shard_bits.
uint32_t FLAGS_ops_per_thread = 2000000;      // Number of operations per thread. (Default: 5 * keyspace size)
...
class KeyGen{
 public:
  ...
  char* CreateKey(uint64_t key){
    for (uint32_t i = 1; i <= FLAGS_key_size; ++i) {
...
class TestCache {
 public:
  TestCache(size_t size):size_(size), cnt_(0), cache_(nullptr){
//    if(FLAGS_hit_rate == 1.0){
//      size *= 16;
//    }

    cache_ = leveldb::NewLRUCache(size);
  }
  bool Get(char* key){
    leveldb::Cache::Handle* handle = cache_->Lookup(leveldb::Slice(key, FLAGS_key_size));
    if (handle != nullptr) {
  ...
  bool Set(char* key){
//    fprintf(stdout, "%s\n", key);
    auto k = leveldb::Slice(key, FLAGS_key_size);
...
//      FLAGS_ops_per_thread = FLAGS_op_num / FLAGS_threads;
    ...
    while (FLAGS_threads * FLAGS_ops_per_thread > 100000000){
      FLAGS_ops_per_thread /= 10;
    }
  }
 ...
 private:
  ...
  void OperateCache(ThreadState *thread) {
    ...
    for (uint64_t i = 0; i < FLAGS_ops_per_thread; i++) {
      uint64_t key_t = thread->rnd.Next() % max_key_;
      bool ans = cache_->Get(kg.CreateKey(key_t));
      if(!ans){
        if(FLAGS_io_time){
          std::this_thread::sleep_for(std::chrono::microseconds (FLAGS_io_time));
        }
        thread->IncMiss();
        cache_->Set(kg.CreateKey(key_t));
//        fprintf(stdout, "%s\n", kg.CreateKey(key_t));
      }
    }
  }
...
bool MyCacheTest(){
  ...
  Slice value;
  cache_test::KeyGen kg;
  assert(c->Set(kg.CreateKey(1)) == true);
//  assert(c->Set(3) == true);
//  assert(c->GetSize() == 1);
  assert(c->Get(kg.CreateKey(1)) == true);
  assert(c->Get(kg.CreateKey(2)) == false);
